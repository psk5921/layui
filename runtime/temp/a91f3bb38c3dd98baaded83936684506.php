<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:92:"F:\phpstudy_pro\WWW\travel_order\public/../application/admin\view\base_info\base_config.html";i:1577773301;s:82:"F:\phpstudy_pro\WWW\travel_order\public/../application/admin\view\common\head.html";i:1577767329;s:84:"F:\phpstudy_pro\WWW\travel_order\public/../application/admin\view\common\footer.html";i:1576813615;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <blockquote class="layui-elem-quote layui-text">
        <div class="demoTable layui-form">
            <div class="layui-input-inline">
                <input type="text" name="name" placeholder="搜索名称" class="layui-input">
            </div>
    ...
    <div class="demoTable layui-form" >
        <div class="layui-tab layui-tab-card" lay-filter="tab_demo">
            <ul class="layui-tab-title">
                <?php foreach($parent_list as $key=>$item): ?>
                <li lay-id="<?php echo $item['code']; ?>" <?php if($key==0): ?> class="layui-this"<?php endif; ?>><?php echo $item['name']; ?></li>
                <?php endforeach; ?>
            </ul>
            <div class="layui-tab-content">
                <div class="layui-tab-item layui-show">
                    <table class="layui-table" lay-data="{height: '', url:'<?php echo (isset($url) && ($url !== '')?$url:''); ?>?code=user_level', page:true, id:'testReload',even: true,loading:true}" lay-filter="demo">
                        <thead>
                        <tr>
                            <th lay-data="{field:'code',width:250,align:'center'}">唯一标识</th>
                            <th lay-data="{field:'name',align:'center'}">名称</th>
                            <th lay-data="{field:'key',align:'center'}">key</th>
                            <th lay-data="{field:'value',align:'center'}">value</th>
                            <th lay-data="{field:'title',align:'center'}">标题</th>
                          <!--  <th lay-data="{field:'content',width:100,align:'center'}">描述</th>-->
                           <!-- <th lay-data="{field:'create_time',width:100,align:'center'}">创建时间</th>
                            <th lay-data="{field:'update_time',width:100,align:'center'}">修改时间</th>-->
                            <th lay-data="{field:'status', templet: '#state',width:100,align:'center'}">状态</th>
    ...
    <script>
        layui.use(['table','layer','form','element'], function(){
                ...
                switch(event){
                    case 'edit':
                        layer.open({
                            ...
                            maxmin: true, //开启最大化最小化按钮
                            area: ['100%', '100%'],
                            content: '<?php echo url("BaseInfo/base_save"); ?>?base_id='+data.base_id,
                            end: function () {
                                table.reload('testReload');
                            },
                        });
                    ...
                    case 'del':
                        layer.confirm('删除后无法恢复,确定删除么', function(index){
                            $.ajax({
                                url:'<?php echo url("BaseInfo/ApiDelBase"); ?>?base_id='+data.base_id,
                                type:'get',
                                dataType:'json',
            ...
            $('.addInfo').on('click', function() {
                var code=$('.layui-this').attr('lay-id');
                layer.open({
                    type: 2,
                    title: '添加信息',
                    shadeClose: true,
                    ...
                    maxmin: true, //开启最大化最小化按钮
                    area: ['100%', '100%'],
                    content: '<?php echo url("BaseInfo/base_add"); ?>?code='+code,
                    end: function () {
                        table.reload('testReload');
                    },
                });
