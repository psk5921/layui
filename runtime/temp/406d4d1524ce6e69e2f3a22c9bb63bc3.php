<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:76:"F:\phpstudy_pro\WWW\layui\public/../application/admin\view\Route\region.html";i:1580726062;s:75:"F:\phpstudy_pro\WWW\layui\public/../application/admin\view\common\head.html";i:1580725934;s:77:"F:\phpstudy_pro\WWW\layui\public/../application/admin\view\common\footer.html";i:1580722672;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<div style="margin: .5rem" class="cbp-spmenu-push">
    <blockquote class="layui-elem-quote layui-text">
        <div class="demoTable layui-form">
            <div class="layui-input-inline">
                <input type="text" name="keyword" class="layui-input" placeholder=" 搜索区域|片区名称">
            </div>
    ...
    <div class="layui-tab layui-tab-card" lay-filter="test" >
        <ul class="layui-tab-title">
            <li lay-id="1" class="layui-this">区域管理</li>
            <li lay-id="2">片区管理</li>
        </ul>
        <div class="layui-tab-content">
            <div class="layui-tab-item layui-show">
                <table class="layui-table" lay-data="{height: '', url:'<?php echo (isset($url1) && ($url1 !== '')?$url1:''); ?>', page:true, id:'testReload1',even: true,loading:true}" lay-filter="demo1">
                    <thead>
                    <tr>
                        <th lay-data="{field:'province_name',align:'center'}">包含省份</th>
                        <th lay-data="{field:'region_name',align:'center'}">区域名称</th>
                        <th lay-data="{field:'region_name_en',align:'center'}">英文名称</th>
                        <th lay-data="{field:'order_list',width:100,align:'center'}">排序</th>
            ...
            <div class="layui-tab-item">
                <table class="layui-table" lay-data="{height: '', url:'<?php echo (isset($url2) && ($url2 !== '')?$url2:''); ?>', page:true, id:'testReload2',even: true,loading:true}" lay-filter="demo2">
                    <thead>
                    <tr>
                        <th lay-data="{field:'area_name',align:'center'}">片区名称</th>
                        <th lay-data="{field:'order_list',width:100,align:'center'}">排序</th>
                        <th lay-data="{field:'state', templet: '#state2',width:100,align:'center'}">状态</th>
                        <th lay-data="{ width:120, align:'center', toolbar: '#barDemo2'}">操作</th>
                    </tr>
                    </thead>
    ...
    layui.use(['table','form','element','layer'], function(){
        ...
        var $ = layui.$, active = {
            reload: function(){
                ...
                var tab_id=$('.layui-this').attr('lay-id');
                if(tab_id==1){
            ...
            switch(event){
                case 'edit':
                    layer.open({
                        ...
                        maxmin: true, //开启最大化最小化按钮
                        area: ['100%', '100%'],
                        content: '<?php echo url("Route/region_save"); ?>?region_id='+data.region_id,
                        end: function () {
                            table.reload('testReload1');
                        },
                    });
                ...
                case 'del':
                    layer.confirm('删除后无法恢复,确定删除么', function(index){
                        $.ajax({
                            url:'<?php echo url("Route/ApiDelRegion"); ?>?region_id='+data.region_id,
                            type:'get',
                            dataType:'json',
            ...
            switch(event){
                case 'edit':
                    layer.open({
                        ...
                        maxmin: true, //开启最大化最小化按钮
                        area: ['100%', '100%'],
                        content: '<?php echo url("Route/area_save"); ?>?area_id='+data.area_id,
                        end: function () {
                            table.reload('testReload2');
                        },
                    });
                ...
                case 'del':
                    layer.confirm('删除后无法恢复,确定删除么', function(index){
                        $.ajax({
                            url:'<?php echo url("Route/ApiDelArea"); ?>?area_id='+data.area_id,
                            type:'get',
                            dataType:'json',
                            success:function(val){
                                if(val.code ==200){
        ...
        $('.addInfo').on('click', function() {
            var tab_id=$('.layui-this').attr('lay-id');
            if(tab_id==1){
                var url='<?php echo url("Route/region_save"); ?>';
                layer.open({
                    type: 2,
                    title: '添加信息',
                    shadeClose: true,
                ...
                var url='<?php echo url("Route/area_save"); ?>';
                layer.open({
                    type: 2,
                    title: '添加信息',
                    shadeClose: true,
