<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:92:"F:\phpstudy_pro\WWW\travel_order\public/../application/admin\view\Order\order_alternate.html";i:1577930070;s:82:"F:\phpstudy_pro\WWW\travel_order\public/../application/admin\view\common\head.html";i:1577767329;s:84:"F:\phpstudy_pro\WWW\travel_order\public/../application/admin\view\common\footer.html";i:1576813615;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<div style="overflow-x: hidden;width: 98%;margin: auto;padding-top: 1rem;">
    <blockquote class="layui-elem-quote layui-text">
        <div class="demoTable layui-form">
            <div class="layui-input-inline">
                <input type="text" name="username" placeholder="队员姓名|手机号|身份证" class="layui-input">
            </div>
            <div class="layui-input-inline">
                <select name="">
                    <option>筛选路线</option>
            ...
            <div class="layui-input-inline">
                <select name="">
                    <option>筛选路线排期</option>
                    <option>20180607-20180708</option>
                    <option>20180607-20180708</option>
                </select>
            </div>
    ...
    <div class="layui-tab layui-tab-card" lay-filter="test" >
        <ul class="layui-tab-title"  >
            <li lay-id="all" class="layui-this">全部订候补</li>
            <li lay-id="1">候补中</li>
            <li lay-id="2">已取消</li>
            <li lay-id="3">完成候补</li>
        </ul>
        <div class="layui-tab-content">
            <div class="layui-tab-item layui-show">
                <table class="layui-table" lay-data="{height: '', url:'<?php echo (isset($url) && ($url !== '')?$url:''); ?>', page:true, id:'testReload',loading:true,skin: 'row',even: true}" lay-filter="demo">
                    <thead>
                    <tr>
                        <th lay-data="{field:'supplement_number',width:180,align:'center'}">候补编号</th>
                        <th lay-data="{field:'order_list',width:80,edit: 'text',align:'center'}">排序</th>
                        <th lay-data="{field:'route_name',width:300,align:'center'}">出团路线</th>
                        <th lay-data="{field:'real_name',templet:'#label',align:'center',width:150}">会员名称</th>
                        <th lay-data="{field:'mobile',width:120,align:'center'}">联系方式</th>
                        <th lay-data="{field:'start_date',width:170,align:'center'}">出团时间</th>
                        <th lay-data="{field:'create_time',width:170,align:'center'}">创建时间</th>
                        <th lay-data="{field:'send_state',width:100,templet: '#send_state',align:'center'}">通知状态</th>
                        <th lay-data="{field:'state',width:120,templet: '#state',align:'center'}">候补状态</th>
                ...
                <script type="text/html" id="barDemo">
                    {{#  if(d.state == 1){ }}
                    <a class="layui-btn layui-btn-xs layui-btn-normal" lay-event="cancel">取消后补</a>
                    {{#  } }}
                </script>
                ...
                <script type="text/html" id="send_state">
                    {{#  if(d.send_state == 1){ }}
                    <span style="color: #5FB878">已通知</span>
                    {{#  } else if( d.send_state ==0) { }}
                    <span style="color: #FF5722">未通知</span>
                    {{#  } }}
                </script>
...
<script>
    ...
    <script>
        layui.use(['table','form','laydate','element'], function(){
            ...
            var $ = layui.$, active = {
                reload: function(){
                    var keyword = $('select[name="keyword"] option:checked').val();
                    //执行重载
                    table.reload('testReload', {
                        where: {
                            keyword:keyword,
            ...
            table.on('edit(demo)', function(obj){
                ...
                $.post('<?php echo url("Order/ApiSaveSupplement"); ?>',{supplement_number:data.supplement_number,order_list:data.order_list},function (res) {
                    layer.msg(res.msg);
                },"JSON")
            });
                ...
                switch(event){
                    case 'cancel':
                        layer.confirm('注意：删除后将无法恢复', function(index){
                            $.post('<?php echo url("Order/ApiSaveSupplement"); ?>',{supplement_number:data.supplement_number,state:2},function (res) {
                                if(res.code==200){
                                    table.reload('testReload');
                                }
                                layer.close(index);
