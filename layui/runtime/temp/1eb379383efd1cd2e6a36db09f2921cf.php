<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:89:"F:\phpstudy_pro\WWW\travel_order\public/../application/admin\view\Coupon\coupon_info.html";i:1577772964;s:82:"F:\phpstudy_pro\WWW\travel_order\public/../application/admin\view\common\head.html";i:1577767329;s:84:"F:\phpstudy_pro\WWW\travel_order\public/../application/admin\view\common\footer.html";i:1576813615;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <blockquote class="layui-elem-quote layui-text">
        <div class="demoTable layui-form">
            <div class="layui-input-inline">
                <input type="text" name="coupon_name" placeholder="搜索优惠券名称" class="layui-input">
            </div>

            <button class="layui-btn" data-type="reload"><i class="layui-icon">&#xe615;</i>搜索</button>
            <button class="layui-btn addInfo"><i class="layui-icon">&#xe654;</i> 添加</button>
        </div>
    </blockquote>

    <table class="layui-table" lay-data="{height: 'full-100', url:'<?php echo (isset($url) && ($url !== '')?$url:''); ?>', page:true, id:'testReload',even: true,loading:true}" lay-filter="demo">
        <thead>
        <tr>
            <th lay-data="{field:'coupon_id',width:100,align:'center'}">ID</th>
            <th lay-data="{field:'coupon_type',templet: '#coupon_type',width:130,align:'center'}">优惠券类型</th>
            <th lay-data="{field:'coupon_name',width:130,align:'center'}">优惠券名称</th>
            <th lay-data="{field:'price',width:100,align:'center'}">优惠金额</th>
            <th lay-data="{field:'use_day',width:100,align:'center'}">有效期(天)</th>
            <th lay-data="{field:'use_range',width:100,templet: '#use_range',align:'center'}">使用范围</th>
            <!--<th lay-data="{field:'describe',align:'center'}">描述</th>-->
         <!--   <th lay-data="{field:'create_time',width:170,align:'center'}">创建时间</th>
            <th lay-data="{field:'update_time',width:170,align:'center'}">修改时间</th>-->
        ...
        <script type="text/html" id="barDemo">
            ...
            <a class="layui-btn layui-btn-xs" lay-event="sel">发放详情</a>
        ...
        <script type="text/html" id="use_range">
            {{#  if(d.use_range == 1){ }}
            <span style="color: #5FB878">全部</span>
            {{#  } else if( d.use_range ==2) { }}
            <span style="color: #FF5722">路线</span>
            {{#  } else if( d.use_range ==3) { }}
            <span style="color: #FF5722">片区</span>
            {{#  } }}
        </script>
        <script type="text/html" id="coupon_type">
            {{#  if(d.coupon_type == 1){ }}
            <span style="color: #5FB878">普通优惠券</span>
            {{#  } }}
        </script>
    </table>
    ...
    <script>
        layui.use(['table','form','element','layer'], function(){
                ...
                switch(event){
                    case 'edit':
                        layer.open({
                            ...
                            maxmin: true, //开启最大化最小化按钮
                            area: ['100%', '100%'],
                            content: '<?php echo url("Coupon/coupon_save"); ?>?coupon_id='+data.coupon_id,
                            end: function () {
                                table.reload('testReload');
                            },
                        });
                    ...
                    case 'sel':
                        layer.open({
                            ...
                            maxmin: true, //开启最大化最小化按钮
                            area: ['100%', '100%'],
                            content: '<?php echo url("Coupon/coupon_user"); ?>?coupon_id='+data.coupon_id,
                            end: function () {
                                table.reload('testReload');
                            },
                        });
                    ...
                    case 'del':
                        layer.confirm('删除后无法恢复,确定删除么', function(index){
                            $.ajax({
                                url:'<?php echo url("Coupon/ApiDelCoupon"); ?>?coupon_id='+data.coupon_id,
                                type:'get',
                                dataType:'json',
            ...
            element.on('tab(tab_demo)', function(elem){
                var regional_type=$(this).attr('lay-id');
                table.reload('testReload', {
                    where: {
            ...
            //添加信息 url跳转路径
            var url='<?php echo url("Coupon/coupon_save"); ?>';
