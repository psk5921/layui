<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:95:"F:\phpstudy_pro\WWW\travel_order\public/../application/admin\view\Route\international_save.html";i:1576813616;s:82:"F:\phpstudy_pro\WWW\travel_order\public/../application/admin\view\common\head.html";i:1577767329;s:84:"F:\phpstudy_pro\WWW\travel_order\public/../application/admin\view\common\footer.html";i:1576813615;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<div style="overflow-x: hidden;width: 96%;margin: auto;padding-top: 1rem;">
    ...
    <div class="layui-form layui-form-pane" lay-filter="example">
        <input type="hidden" name="international_id" value="<?php echo (isset($info['international_id']) && ($info['international_id'] !== '')?$info['international_id']:''); ?>">
        <div class="layui-form-item">
            <label class="layui-form-label">洲名称</label>
            <div class="layui-input-inline">
                <input type="text" name="name" value="<?php echo (isset($info['name']) && ($info['name'] !== '')?$info['name']:''); ?>" class="layui-input">
            </div>
            <label class="layui-form-label">英文名称</label>
            <div class="layui-input-inline">
                <input type="text" name="name_en" value="<?php echo (isset($info['name_en']) && ($info['name_en'] !== '')?$info['name_en']:''); ?>" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
    ...
    <div class="layui-form layui-form-pane" lay-filter="example">
        ...
        <div class="layui-form-item">
            <label class="layui-form-label">洲名称</label>
            <div class="layui-input-inline">
                <select name="parent_id" class="continent_list">
        ...
        <div class="layui-form-item">
            <p style="color: red">*请上传1600x500px的图片</p>
        ...
        <div class="layui-form-item">
            <p style="color: red">*请上传750x860px的图片</p>
        ...
        <div class="layui-form-item">
            <p style="color: red">*请上传280x280的视频</p>
        ...
        <div class="layui-form-item">
            <div class="layui-input-block">
                <button class="layui-btn" lay-submit="" lay-filter="submit2">立即提交</button>
                <button type="reset" class="layui-btn layui-btn-primary close">关闭</button>
            </div>
        </div>
    </div>
...
<script>
    layui.use(['form'], function() {
        ...
        form.on('submit(submit1)', function(data){
            $.post('<?php echo url("Route/APiSaveContinentInfo"); ?>',data.field,function (res) {
                if(res.code==200){
        ...
        //获取洲列表
        $.get('<?php echo url("Route/ApiGetContinentPage"); ?>',{limit:100},function (res) {
            var parent_id="<?php echo (isset($info['parent_id']) && ($info['parent_id'] !== '')?$info['parent_id']:''); ?>";
            if(res.code==0){
                var data=res.data;
                var html='';
                $.each(data,function (k,v) {
                    if(parent_id == v.international_id){
                        html+='<option value="'+v.international_id+'" selected>'+v.name+'　'+v.name_en+'</option>';
                    }else{
                        html+='<option value="'+v.international_id+'">'+v.name+'　'+v.name_en+'</option>';
                    }
                })
                $('.continent_list').html(html);
                form.render(); //更新全部
            }
        },'JSON')
