<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:92:"F:\phpstudy_pro\WWW\travel_order\public/../application/admin\view\Route\route_type_edit.html";i:1577155323;s:82:"F:\phpstudy_pro\WWW\travel_order\public/../application/admin\view\common\head.html";i:1576813615;s:84:"F:\phpstudy_pro\WWW\travel_order\public/../application/admin\view\common\footer.html";i:1576813615;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<div style="height:48rem;overflow-x: hidden;width: 96%;margin: auto;padding-top: 1rem;">
    ...
    <div class="layui-form layui-form-pane">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input type="text" value="<?php echo (isset($info['name']) && ($info['name'] !== '')?$info['name']:''); ?>" name="name" lay-verify="required"  class="layui-input">
            </div>
        </div>

        <div class="layui-form-item">

            <p style="color: red">*请上传180x260px的图片</p>
            <label class="layui-form-label">顶部图片</label>
            <div class="layui-input-block">
                <div id="top_images">
                    <?php if(isset($info) && !empty($info['top_images'])): foreach($info['top_images'] as $item): ?>
                    <div class="file_img" data-type="img">
                        <i class="layui-icon del_file">&#x1007;</i>
                        <img src="<?php echo $item['url']; ?>">
                        <input type="hidden" value="<?php echo $item['file_id']; ?>" data-url="<?php echo $item['url']; ?>" name="top_images[<?php echo $item['file_id']; ?>]"/>
                    </div>
                    <?php endforeach; endif; ?>
                </div>
                <button class="layui-btn add_file" data-id="top_images">选择图片</button>
            </div>
        </div>
        <div class="layui-form-item">
            <p style="color: red">*请上传180x260px的图片</p>
            <label class="layui-form-label">中间banner图</label>
            <div class="layui-input-block">
                <div id="banner_images">
                    <?php if(isset($info) && !empty($info['banner_images'])): foreach($info['banner_images'] as $item): ?>
                    <div class="file_img" data-type="img">
                        <i class="layui-icon del_file">&#x1007;</i>
                        <img src="<?php echo $item['url']; ?>">
                        <input type="hidden" value="<?php echo $item['file_id']; ?>" data-url="<?php echo $item['url']; ?>" name="banner_images[<?php echo $item['file_id']; ?>]"/>
                    </div>
                    <?php endforeach; endif; ?>
                </div>
                <button class="layui-btn add_file" data-id="banner_images">选择图片</button>
            </div>
        </div>
        <div class="layui-form-item">
            <p style="color: red">*请上传617x415px的图片</p>
            <label class="layui-form-label">主图展示</label>
            <div class="layui-input-block">
                <div id="main_images">
                    <?php if(isset($info) && !empty($info['main_images'])): foreach($info['main_images'] as $item): ?>
                    <div class="file_img" data-type="img">
                        <i class="layui-icon del_file">&#x1007;</i>
                        <img src="<?php echo $item['url']; ?>">
                        <input type="hidden" value="<?php echo $item['file_id']; ?>" data-url="<?php echo $item['url']; ?>" name="main_images[<?php echo $item['file_id']; ?>]"/>
                    </div>
                    <?php endforeach; endif; ?>
                </div>
                <button class="layui-btn add_file" data-id="main_images">选择图片</button>
            </div>
        </div>
    ...
    layui.use(['form', 'layedit', 'laydate'], function(){
       var form = layui.form
            ...
            form.on('submit(submit)', function(data){
                    $.post('<?php echo url("route_type_edit"); ?>',data.field,function (res) {
                        if(res.code==200){
